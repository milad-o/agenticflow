...
class ToolRepo:
    ...
    def _register_defaults(self) -> None:
        ...
        # CSV (optional)
        try:
            from agenticflow.tools.csv_tools import MergeCsvTool, ValidateCsvJoinTool, CsvChunkAggregateTool, PandasChunkAggregateTool  # noqa: F401
        ...
        try:
            from agenticflow.tools.ephemeral_chroma import (
                BuildEphemeralChromaTool,
                QueryEphemeralChromaTool,
            )

            def _build_ephemeral() -> BaseTool:
                return BuildEphemeralChromaTool()

            def _query_ephemeral() -> BaseTool:
                builder = BuildEphemeralChromaTool()
                return QueryEphemeralChromaTool(builder)

            self._register(
                RepoToolMeta(
                    name="build_ephemeral_chroma",
                    description="Build an ephemeral Chroma index in-memory",
                    tags={"index", "retrieval", "vector"},
                    capabilities={"retrieval"},
                    factory=_build_ephemeral,
                )
            )
            self._register(
                RepoToolMeta(
                    name="query_ephemeral_chroma",
                    description="Query the ephemeral Chroma index",
                    tags={"index", "retrieval", "vector"},
                    capabilities={"retrieval"},
                    factory=_query_ephemeral,
                )
            )
        except Exception:
            pass
        ...
        # Vector set if available  
        if "build_ephemeral_chroma" in self._tools:
            self._sets["vector"] = ["build_ephemeral_chroma", "query_ephemeral_chroma"]
