...
from agenticflow.core.event_bus import get_event_bus
import structlog
...
"""
    ...
    async def _plan_heuristic(self, state: HybridRPAVHState) -> Dict[str, Any]:
        ...
        if any(pattern in request for pattern in ["average", "avg", "mean", "group by", "per category", "aggregate"]):
            if "csv_chunk_aggregate" in available_tools:
            ...
            for action in pending_actions:
                ...
                try:
                    ...
                    if tool_name == "read_text_fast" and "path" not in params:
                        ...
                        # Build a structured Markdown report (heuristic baseline)
                        content_parts = []
                        content_parts.append("# Analysis Report\n\n")
                        content_parts.append(f"**Request**: {state['original_request'].split('\n')[0]}\n\n")

                        # Executive summary
                        total_files = len(files_info)
                        total_size = sum(int(f.get("size_bytes", 0)) for f in files_info)
                        content_parts.append("## Executive Summary\n\n")
                        content_parts.append(f"- Packages discovered: {total_files}\n")
                        if total_files:
                            content_parts.append(f"- Total size: {total_size/1024:.1f} KB\n\n")
                        else:
                            content_parts.append("- No files discovered.\n\n")

                        # Data overview as a table
                        if files_info:
                            content_parts.append("## Data Overview\n\n")
                            content_parts.append("| File | Size (KB) |\n")
                            content_parts.append("|------|-----------|\n")
                            for f in files_info[: self.static_resources.get("max_files_to_include", 10)]:
                                path = f.get("path", "")
                                size_kb = (int(f.get("size_bytes", 0)) / 1024.0)
                                content_parts.append(f"| {Path(path).name} | {size_kb:.1f} |\n")
                            content_parts.append("\n")

                        # Heuristic key findings from content and filenames
                        findings = []
                        combined_text = "\n".join(content_snippets).lower()
                        if "archive" in combined_text:
                            findings.append("Detected archiving operations (e.g., moving or archiving files).")
                        if "transform" in combined_text or "staging" in combined_text:
                            findings.append("Detected data transformation or staging steps.")
                        if "extract" in combined_text or "raw" in combined_text:
                            findings.append("Detected raw data extraction steps.")
                        try:
                            for f in files_info:
                                name = Path(f.get("path", "")).stem.lower()
                                if "archive" in name and not any("archiving" in x for x in findings):
                                    findings.append("Package names indicate archiving/cleanup operations.")
                                if "transform" in name and not any("transformation" in x for x in findings):
                                    findings.append("Package names indicate transformation or staging steps.")
                                if "extract" in name and not any("extraction" in x for x in findings):
                                    findings.append("Package names indicate raw data extraction steps.")
                        except Exception:
                            pass
                        content_parts.append("## Key Findings\n\n")
                        if findings:
                            for fnd in findings:
                                content_parts.append(f"- {fnd}\n")
                        else:
                            content_parts.append("- No obvious findings from quick scan.\n")
                        content_parts.append("\n")

                        # Detailed analysis (lightweight, heuristic)
                        if files_info or aggregates:
                            content_parts.append("## Detailed Analysis\n\n")
                            # Include aggregate table if present
                            if aggregates:
                                try:
                                    agg = aggregates[0]
                                    content_parts.append(f"Computed {agg.get('agg','mean')} of {agg.get('value_column','value')} grouped by {agg.get('group_by','group')}. Top groups by count shown.\n\n")
                                    content_parts.append("| Group | Count | Average |\n")
                                    content_parts.append("|-------|-------:|--------:|\n")
                                    for g in agg.get("groups", [])[:10]:
                                        content_parts.append(f"| {g.get('group','')} | {int(g.get('count',0))} | {float(g.get('average',0.0)):.4f} |\n")
                                    content_parts.append("\n")
                                except Exception:
                                    pass
                            for f in files_info[: self.static_resources.get("max_files_to_include", 10)]:
                                ...
                                low = name.lower()
                                if "archive" in low:
                                    label.append("Archive")
                                if "transform" in low or "staging" in low:
                                    label.append("Transform")
                                if "extract" in low or "raw" in low:
                                    label.append("Extract")
                                tag = f" ({', '.join(label)})" if label else ""
                                content_parts.append(f"- {name}: {size_kb:.1f} KB{tag}\n")
                            content_parts.append("\n")

                        # Content samples (as evidence)
                        if content_snippets:
                            content_parts.append("## Content Samples\n\n")
                            max_chars = self.static_resources.get("content_limits", {}).get("max_file_content_chars", 1000)
                            for i, snippet in enumerate(content_snippets[:3], 1):
                                content_parts.append(f"### Sample {i}\n\n")
                                clean = snippet[:max_chars]
                                content_parts.append(f"```\n{clean}\n```\n\n")

                        # Conclusion
                        content_parts.append("## Conclusion\n\n")
                        if total_files:
                            content_parts.append(f"Scanned {total_files} SSIS package(s). This is a fast heuristic report. ")
                            content_parts.append("Enable LLM synthesis for a richer executive summary, detailed analysis, and recommendations.\n\n")
                        else:
                            content_parts.append("No packages were discovered. Provide a different search path or pattern to continue.\n\n")

                        # Footer
                        content_parts.append(f"**Generated at**: {time.ctime()}\n")
