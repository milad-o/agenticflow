...
from agenticflow.orchestration.delegation.delegate_tool import DelegateTool
from agenticflow.orchestration.orchestrators.orchestrator_tools import CreateFileTool, ListDirectoryTool
from agenticflow.orchestration.planners.planner import Planner, Plan
from agenticflow.orchestration.planners.splitter import split_atomic
from agenticflow.orchestration.delegation.capability_extractor import CapabilityExtractor
from agenticflow.core.tasks import TaskStatusTracker, TaskStatus
from agenticflow.core.events import get_event_bus, EventType, Event
import structlog
                ...
                try:
                    ...
                    if chosen is None and any(w in dlow for w in ["average", "avg", "mean", "aggregate", "group by", "per category"]):
                        for name, agent in self.agents.items():
                            ...
                            if any(n in ("pandas_chunk_aggregate", "csv_chunk_aggregate") for n in tool_names):
                                chosen = name
                                if getattr(self, "reporter", None):
