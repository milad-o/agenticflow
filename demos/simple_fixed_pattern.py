#!/usr/bin/env python3
"""Simple fixed LangGraph pattern following the tutorial exactly."""
...
# Create a ReAct-style search agent like in the tutorial
...
def search_node(state: State) -> Command[Literal["supervisor"]]:
    ...
    return Command(
        update={
            "messages": [
                HumanMessage(content=last_message.content, name="search")
            ]
        },
        goto="supervisor",
    )
...
async def test_simple_pattern():
    ...
    print("🔍 Testing Simple Fixed LangGraph Pattern")
    print("=" * 50)
